...
async function convert(i) {
	...
	if (toConvert.type == "smi") {
		...
		const assText = SmiFile.holdsToAss(holds, appendParts, append.getStyles().body, append.getEvents().body, x, y).toText(true);
...
window.addEventListener("load", () => {
	setTimeout(() => {
		...
		link.rel = "stylesheet";
		link.href = new URL("./Viewer.css?260605", import.meta.url).href;
