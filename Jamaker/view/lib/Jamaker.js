	...
	{	const holds = SmiFile.textToHolds(text);
		...
		const assHold = tab.assHold = new SmiEditor();
		{
			...
			{	const holdName = document.createElement("div");
				...
				span.innerText = "ASS 추가 스크립트";
				holdName.append(span);
				assHold.selector.append(holdName);
			}
			assHold.selector.title = "ASS 추가 스크립트";
			assHold.owner = this;
			...
			assHold.assEditor.onUpdate = function() {
				if (this.isSaved) {
					assHold.selector.classList.remove("not-saved");
				} else {
			...
			tab.area.querySelector("div.tab-ass-script button.btn-split-hold").addEventListener("click", () => {
		...
		if (this.assFile) {
			...
			this.assFile.parts.forEach((part) => {
				if (part.name == "Script Info") return;
				if (part.name == "Events") return;
...
Tab.prototype.addAutomation = function(item) {
	...
	if (item) {
		...
		cm.setValue(
			...
			+	",	text: 해당 조각의 문자열\n"
			+	",	top: 해당 조각의 karaoke.y 기준 상대 좌표\n"
			+	",	left: 해당 조각의 karaoke.x 기준 상대 좌표\n"
			+	",	width: 해당 조각의 좌우 폭\n"
			+	"}\n"
...
Tab.prototype.updateHoldSelector = function() {
	...
	{	const begins = []; // 각 홀드의 시작 위치를 기록한 객체
		timers.forEach((timer, i) => {
			timer.holds.forEach((selector) => {
				if (selector.type == END) {
					// 홀드 종료 위치가 시작 위치와 4칸 이상 떨어져야 함
					const min = begins[selector.index] + 4;
					if (i + add < min) {
						// 4칸 안 될 경우 보정값에 추가
						add = min - i;
					}
			...
			timer.holds.forEach((selector) => {
				if (selector.type == BEGIN) {
					...
					begins[selector.index] = timer.rate;
				}
			});
		});
	}
...
Tab.prototype.getAdditionalToAss = function(forSave=false) {
	...
	if (appends.trim().length) {
		...
		appendFile.parts.forEach((appendPart) => {
			if (appendPart.name == "Script Info") return;
			if (appendPart.name == "Events") return;
			if (appendPart.name == "V4+ Styles") {
				const part = assFile.getStyles();
				appendPart.body.forEach((appendStyle) => {
					let style = null;
...
Tab.prototype.onChangeSaved = function(hold) {
	if (this.isSaved()) {
		...
		this.holds.forEach((hold) => {
			hold.selector.classList.remove("not-saved");
		});
		...
		if (!Array.isArray(setting)) {
			...
			if (setting.version != DEFAULT_SETTING.version) {
				...
				if (notified.menu) {
					...
					DEFAULT_SETTING.menu.forEach((dMenu) => {
						let exist0 = false;
		...
		fetch("lib/Jamaker.color.css").then(async (response) => {
			...
			function setStyleWithHighlight() {
				...
				if (setting.highlight && setting.highlight.parser) {
					let hljs = document.getElementsByClassName("hljs");
					if (hljs.length) {
						hljs = hljs[0];
					} else {
						if (window.tempHljs) {
...
window.saveFile = function(asNew, isExport) {
	...
	setTimeout(async () => {
		...
		currentTab.holds.forEach((hold, i) => {
			hold.remember();
		...
		if (withAss) {
			...
			const assStyles = {};
			{
				appendStyles.body.forEach((assStyle) => {
					const arrStyle = [];
			...
			appendStyles.body.forEach((item) => {
				if (item.hasHold) {
					removeCount++;
				} else {
					requiredStyles.push(item);
				}
			});
			if (removeCount) {
				...
				if (addCount + delCount) {
					...
					while (ti < list.length) {
						...
						for (let s = 0; !fromAssEditor && s < styles.length; s++) {
							...
							// style 기준으로 홀드 찾기
							currentTab.holds.forEach((hold, h) => {
					...
					currentTab.holds.forEach((hold) => {
						...
						for (let i = 0; i < hold.lines.length; i++) {
							...
							if (line.TYPE && (line.SYNC >= cursorSync)) {
								break;
							}
						}
					...
					appendEvents.body.forEach((event) => {
...
window.fitSyncsToFrame = function(frameSyncOnly=false, add=0) {
	...
	holds.forEach((hold) => {
		hold.fitSyncsToFrame(frameSyncOnly, add);
	});
	tabs[tabIndex].assHold.fitSyncsToFrame();
}
...
window.runColorPicker = function(useWvPicker=false) {
	...
	do { // #RRGGBB 찾기
		...
		if (begin < 0) {
			...
			let skip = 0;
			let next = -1;
			while ((next = line.indexOf("#", skip)) >= 0) {
		...
		value = rgb;
		editor.cm.setSelection({ line: lineNo, ch: begin }, { line: lineNo, ch: begin+7 });
		found = begin;
	} while (false);
	...
	input.value = input.nextSibling.value = "#" + value;
