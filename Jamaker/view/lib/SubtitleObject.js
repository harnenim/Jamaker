...
AssEvent.inFromAttrs = (attrs, checkFurigana=true, checkFade=true, checkAss=true, last=null) => {
	...
	// 색상to색상인 경우 - 겹치는 객체 만들어서 처리해야 함 -> TODO: 문제없으면 \t 태그 기반으로 변경
	const colorFadeByT = true;
	if (checkFade) {
		let count = 0;
		let countHides = 0;
		const baseAttrs = [];
		...
		if (count) {
			...
			if (countHides) {
				...
				if (colorFadeByT) {
					...
					baseAttrs.forEach((base, i) => {
						...
						if (!wasHide) {
							const orig = attrs[i];
							if (typeof orig.fade == "string" && orig.fade[0] == "#" && orig.fade.length == 7) {
								// 색상 페이드 최종 색
								base.fc = orig.fade.substring(1);
							}
						}
						base.text = tag + base.text;
					});
			...
			if (colorFadeByT) {
				...
				attrs.forEach((orig, i) => {
					const attr = new Attr(orig, orig.text);
					let isFade = false;
					if (typeof attr.fade == "string" && attr.fade[0] == "#" && attr.fade.length == 7) {
						// 페이드 생성
						const fadeAttr = attr.clone();
						fadeAttr.text = `{\\t(\\c&H${attr.fade.substring(5,7)}${attr.fade.substring(3,5)}${attr.fade.substring(1,3)}&)}` + fadeAttr.text;
						fadeAttrs.push(fadeAttr);
...
window.AssFile = Subtitle.AssFile = function(text, width=0, height=0) {
	this.parts = [];
	if (text) {
		this.fromText(text);
	} else {
		...
		scriptInfo.body.push({ key: "PlayResX", value: width  ? width  : Subtitle.video.width});
		scriptInfo.body.push({ key: "PlayResY", value: height ? height : Subtitle.video.height });
