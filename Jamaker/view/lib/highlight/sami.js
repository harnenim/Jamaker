...
CodeMirror.defineMode("sami", function(editorConf, config_) {
	...
	function inText(stream, state) {
		...
		if (ch == "<") {
			if (stream.eat("!")) {
				if (stream.eat("[")) {
					if (stream.match("CDATA[")) return chain(inBlock("hljs-atom", "]]>"));
					else return null;
				} else if (stream.match("--")) {
					return chain(inBlock("hljs-comment", "-->"));
...
//		if (!config.allowMissing) setStyle = "error";
	...
	function attrValueState(type, stream, state) {
		...
		if (type == "word" && config.allowUnquoted) {setStyle = "hljs-value"; return attrState;}
//		setStyle = "error";
		return attrState(type, stream, state);
	}
	...
	const result = {
		...
		token: function(stream, state) {
			...
			if ((style || type) && style != "hljs-comment") {
				setStyle = null;
...
CodeMirror.defineMIME("text/sami", "sami");
