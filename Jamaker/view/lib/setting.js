...
,	command:
	...
	,	withCtrls:
		...
		,	'2': '/* 한 줄씩 줄표 넣어주기 */\n'
			   ...
			   + '	// 줄표가 이미 있으면 없앨 수 있는지 확인\n'
			   + '	const textLines = lines[syncLineNo + 1].split(/<br>/gi);\n'
			   + '	for (let i = 0; i < textLines.length; i++) {\n'
			   + '		textLines[i] = textLines[i].substring(1).trim();\n'
			   + '	}\n'
			   + '	let cursor = lines.slice(0, syncLineNo + 1).join("\\n").length + 1;\n'
			   + '	lines.splice(syncLineNo + 1, 1, ...textLines);\n'
	...
	,	withCtrlAlts:
		...
		,	'X': '/* ASS clip x 좌표 이동 */\n'
			   ...
			   + '	add = Number(add);\n'
			   + '	\n'
			   + '	let values = line.substring(begin, end).trim();\n'
			   + '	if (values.startsWith("m")) {\n'
			   + '		values = values.replaceAll("  ", " ").split(" ");\n'
			   + '		let d = 0;\n'
			   + '		for (let i = 0; i < values.length; i++) {\n'
			   + '			if (isFinite(values[i])) {\n'
			   + '				if (i % 2 == d) { // x 좌표 차례\n'
			   + '					values[i] = Number(values[i]) + add;\n'
			   + '				}\n'
			   + '			} else {\n'
			   ...
			   + '	}\n'
			   + '	\n'
			   + '	editor.setLine(line.substring(0, begin) + values + line.substring(end));\n'
			   + '});'
		,	'Y': '/* ASS clip y 좌표 이동 */\n'
			   + 'const line = editor.getLine().text;\n'
			   + 'let begin = line.indexOf("\\clip(");\n'
